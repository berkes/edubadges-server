...
class StudentsEnrolledList(BaseEntityListView):
    ...
    def post(self, request, **kwargs):
        ...
        if request.user.may_enroll(badge_class, raise_exception=True):
            ...
            badge_class.remove_cached_data(
                ["cached_pending_enrollments_including_denied"]
            )
            ...
            def send_notifications(badge_class, created_enrollment):
                user_notifications = BadgeClassUserNotification.objects.filter(
                ...
                for user_notification in user_notifications:
                    ...
                    if perms["may_sign"]:
                        html_message = (
                            EmailMessageMaker.create_enrolment_notification_mail(
            ...
            thread = threading.Thread(
                target=send_notifications, args=(badge_class, enrollment)
            )
            thread.start()
            return Response(
                data={"status": "enrolled", "entity_id": enrollment.entity_id},
                status=201,
            )
...
class EnrollmentDetail(BaseEntityDetailView):
    ...
    def put(self, request, **kwargs):
        ...
        subject = (
            "Your request for the badgeclass {} has been denied by the issuer.".format(
        ...
        enrollment.badge_class.remove_cached_data(
            ["cached_pending_enrollments_including_denied"]
        )
