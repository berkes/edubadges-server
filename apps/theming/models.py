...
def get_current_templates():
    ...
    for template in os.listdir(
        os.path.join(
            settings.TOP_DIR, "apps", "mainsite", "templates", "terms_of_service"
        )
    ):
        templates.append(
            (
                os.path.join("terms_of_service", template),
                os.path.join("terms_of_service", template),
            )
        )
...
class Theme(models.Model):
    ...
    badgr_app = models.OneToOneField(
        BadgrApp, on_delete=models.SET_NULL, null=True, related_name="theme"
    )
    institution = models.ForeignKey(
        Institution, on_delete=models.CASCADE, related_name="theme"
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._meta.get_field("terms_and_conditions_template")._choices = lazy(
            get_current_templates, list
        )()
