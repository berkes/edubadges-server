...
class InstitutionForm(ModelForm):
    ...
    def clean(self):
        sis_integration_enabled = self.cleaned_data.get(
            "sis_integration_enabled", False
        )
        if sis_integration_enabled:
            sis_default_user = self.cleaned_data.get("sis_default_user", None)
            if not sis_default_user:
                self._errors["sis_default_user"] = self.error_class(
                    ["sis_default_user required for SIS integration"]
                )
            manage_client_id = self.cleaned_data.get("manage_client_id", None)
            if not manage_client_id:
                self._errors["manage_client_id"] = self.error_class(
                    ["manage_client_id required for SIS integration"]
                )
...
class InstitutionAdmin(admin.ModelAdmin):
    ...
    def formfield_for_foreignkey(self, db_field, request=None, **kwargs):
        if db_field.name == "sis_default_user":
            ...
            if institution_id:
                kwargs["queryset"] = BadgeUser.objects.filter(
                    is_teacher=True, institution__id=institution_id
                )
        return super(InstitutionAdmin, self).formfield_for_foreignkey(
