...
class InstitutionSerializer(InternalValueErrorOverrideMixin, serializers.Serializer):
    ...
    name_dutch = serializers.CharField(
        max_length=254, required=False, allow_null=True, allow_blank=True
    )
    grading_table = serializers.URLField(
        max_length=254, required=False, allow_null=True, allow_blank=True
    )
    award_allowed_institutions = PrimaryKeyRelatedField(
        many=True, queryset=Institution.objects.all(), required=False
    )
    linkedin_org_identifier = serializers.CharField(
        max_length=254, required=False, allow_null=True, allow_blank=True
    )
    ...
    manage_client_id = serializers.CharField(
        max_length=254, required=False, allow_null=True, allow_blank=True
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["sis_default_user"].queryset = BadgeUser.objects.filter(
            is_teacher=True
        )
        ...
        if not data.get("image_english", False) and not data.get("image_dutch", False):
            e = OrderedDict(
                [
                    (
                        "image_english",
                        [ErrorDetail("English or Dutch image is required", code=926)],
                    )
                ]
            )
            errors = OrderedDict(chain(errors.items(), e.items()))
            e = OrderedDict(
                [
                    (
                        "image_dutch",
                        [ErrorDetail("Dutch or English image is required", code=918)],
                    )
                ]
            )
            errors = OrderedDict(chain(errors.items(), e.items()))
        if not data.get("description_english", False) and not data.get(
            "description_dutch", False
        ):
            e = OrderedDict(
                [
                    (
...
class FacultySerializer(InternalValueErrorOverrideMixin, serializers.Serializer):
    ...
    on_behalf_of_url = serializers.URLField(
        max_length=512, required=False, allow_null=True, allow_blank=True
    )
    on_behalf_of_display_name = serializers.CharField(
        max_length=512, required=False, allow_null=True, allow_blank=True
    )
        ...
        if not data.get("name_english", False) and not data.get("name_dutch", False):
            ...
            e = OrderedDict(
                [
                    (
                        "name_dutch",
                        [ErrorDetail("English or Dutch name is required", code=912)],
                    )
                ]
            )
            errors = OrderedDict(chain(errors.items(), e.items()))
            ...
            e = OrderedDict(
                [
                    (
                        ...
                        [
                            ErrorDetail(
                                "English or Dutch description is required", code=913
                            )
                        ],
                    )
                ]
