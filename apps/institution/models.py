...
class Institution(
    ...
    staff = models.ManyToManyField(
        "badgeuser.BadgeUser", through="staff.InstitutionStaff", related_name="+"
    )
    ...
    image_english = models.FileField(
        upload_to="uploads/institution", blank=True, null=True
    )
    image_dutch = models.FileField(
        upload_to="uploads/institution", blank=True, null=True
    )
    grading_table = models.CharField(
    ...
    award_allow_all_institutions = models.BooleanField(
        default=False, help_text="Allow awards to all institutions"
    )
    public_institution = models.BooleanField(
        default=True, help_text="Set to false to exclude from the catalog"
    )

    lti_enabled = models.BooleanField(
        default=False, help_text="Set to true to enable LTI for this institution"
    )
    alternative_identifier = models.CharField(
        max_length=255,
        blank=True,
        null=True,
        default=None,
    ...
    eppn_reg_exp_format = models.CharField(
        max_length=255,
        blank=True,
        null=True,
        default=None,
    ...
    linkedin_org_identifier = models.CharField(
        max_length=255,
        blank=True,
        null=True,
        default=None,
    ...
    manage_client_id = models.CharField(
        max_length=255,
        blank=True,
        null=True,
        default=None,
    ...
    grondslag_formeel = models.CharField(
        max_length=254,
        null=True,
        blank=True,
        choices=GRONDSLAG_CHOICES,
        default=GRONDSLAG_UITVOERING_OVEREENKOMST,
    )
    grondslag_informeel = models.CharField(
        max_length=254,
        null=True,
        blank=True,
        choices=GRONDSLAG_CHOICES,
    ...
    institution_type = models.CharField(
        max_length=254, null=True, blank=True, choices=TYPE_CHOICES
    )
    ...
    default_language = models.CharField(
        max_length=254, choices=DEFAULT_LANGUAGE_CHOICES, default=DEFAULT_LANGUAGE_DUTCH
    )
    ...
    def clean(self):
        if self.eppn_reg_exp_format:
            try:
                ...
                if not (
                    self.eppn_reg_exp_format.startswith(".*")
                    or self.eppn_reg_exp_format.startswith("(.*")
                ):
                    raise ValidationError(
                        "Eppn reg exp format must start with '.*' or (.*)"
                    )
    ...
    def get_report(self):
        ...
        return {
            ...
            "total_admins": [
                staff
                for staff in self.cached_staff()
                if staff.permissions == staff.full_permissions()
            ].__len__(),
    ...
    def get_faculties(self, user, permissions):
        return [
            fac
            for fac in self.cached_faculties()
            if fac.has_permissions(user, permissions)
        ]
    ...
    def validate_unique(self, exclude=None):
        ...
        institution_same_name = (
            self.__class__.objects.filter(query).exclude(pk=self.pk).first()
        )
        if institution_same_name:
            name_english_the_same = (
                institution_same_name.name_english == self.name_english
                and bool(institution_same_name.name_english)
            )
            name_dutch_the_same = (
                institution_same_name.name_dutch == self.name_dutch
                and bool(institution_same_name.name_dutch)
            )
    ...
    def create_staff_membership(self, user, permissions):
        try:
            ...
            return InstitutionStaff.objects.create(
                user=user, institution=self, **permissions
            )
    ...
    def get_json(self, obi_version, expand_awards=False):
        ...
        image_url = OriginSetting.HTTP + reverse(
            "institution_image", kwargs={"entity_id": self.entity_id}
        )
        ...
        if expand_awards:
            ...
            json["award_allowed_institutions"] = [
                inst.name for inst in self.award_allowed_institutions.all()
            ]
        return json
...
class BadgeClassTag(models.Model):
    institution = models.ForeignKey(
        Institution, on_delete=models.CASCADE, blank=False, null=False
    )
...
class Faculty(
    ...
    on_behalf_of_url = models.CharField(
        max_length=512, blank=True, null=True, default=None
    )
    on_behalf_of_display_name = models.CharField(
        max_length=512, blank=True, null=True, default=None
    )
            ...
            if not self.archived:
                ...
                faculty_same_name = (
                    self.__class__.objects.filter(
                        query, institution=self.institution, archived=False
                    )
                    .exclude(pk=self.pk)
                    .first()
                )
                if faculty_same_name:
                    name_english_the_same = (
                        faculty_same_name.name_english == self.name_english
                        and bool(faculty_same_name.name_english)
                    )
                    name_dutch_the_same = (
                        faculty_same_name.name_dutch == self.name_dutch
                        and bool(faculty_same_name.name_dutch)
                    )
                    both_the_same = name_english_the_same and name_dutch_the_same
                    if both_the_same:
                        raise BadgrValidationMultipleFieldError(
    ...
    def get_issuers(self, user, permissions):
        return [
            issuer
            for issuer in self.cached_issuers()
            if issuer.has_permissions(user, permissions)
        ]
