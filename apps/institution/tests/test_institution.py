...
class InstitutionTest(BadgrTestCase):
    def test_create_faculty(self):
        ...
        response = self.client.post(
            "/institution/faculties/create",
            data=json.dumps(faculty_json),
    ...
    def test_edit_institution(self):
        ...
        response = self.client.put(
            "/institution/edit/{}".format(teacher1.institution.entity_id),
    ...
    def test_check_institutions_validity(self):
        ...
        response = self.client.post(
            "/institution/check",
            data=json.dumps([teacher1.institution.identifier]),
            content_type="application/json",
        )
        self.assertTrue(response.data[0]["valid"])
        response = self.client.post(
            "/institution/check",
            data=json.dumps(["NOT EXIST"]),
            content_type="application/json",
        )
    ...
    def test_faculty_delete(self):
        ...
        response_fail = self.client.delete(
            "/issuer/faculty/delete/{}".format(faculty.entity_id),
            content_type="application/json",
        )
        self.assertEqual(response_fail.status_code, 404)
        assertion.delete()
        response_success = self.client.delete(
            "/institution/faculties/delete/{}".format(faculty.entity_id),
            content_type="application/json",
        )
