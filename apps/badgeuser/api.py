...
from badgeuser.serializers import (
    BadgeUserProfileSerializer,
    UserProvisionmentSerializer,
        ...
        for provisionment in request.data:
            ...
            try:
                ...
                duplicate_with_success = [
                    x
                    for x in response
                    if x["status"] == "success" and x["email"] == provisionment["email"]
                ]
...
class UserProvisionmentDetail(BaseEntityDetailView):
    ...
    permission_map = {
        "PUT": "may_administrate_users",
        "DELETE": "may_administrate_users",
    }
...
class AcceptProvisionmentDetail(BaseEntityDetailView):
    ...
    def post(self, request, **kwargs):
        obj = self.get_object(request, **kwargs)
        if not self.has_object_permissions(
            request, obj
        ):  # checks if obj is owned by request.user
...
class AcceptTermsView(APIView):
    ...
    def post(self, request, **kwargs):
        if request.data:
            serializer = TermsAgreementSerializer(
...
class PublicTermsView(APIView):
    ...
    def get(self, request, **kwargs):
        ...
        if data:
            return Response(
                TermsSerializer(many=True).to_representation(data), status=HTTP_200_OK
            )
        else:
            return Response(status=HTTP_404_NOT_FOUND)
