    ...
    def post(self, request, **kwargs):
        ...
        try:  # check if email already exists
            ...
            raise BadgrApiException400(
                "Could not register email address. Address already in use", 101
            )
        except CachedEmailAddress.DoesNotExist:
            try:
                CachedEmailAddress.objects.get(
                    email=request.data.get("email"), verified=0, user_id=request.user.pk
                )
                raise BadgrApiException400(
                    "You have already added this address. Verify it", 102
                )
            except CachedEmailAddress.DoesNotExist:
                pass
...
class BadgeUserEmailDetail(BadgeUserEmailView):
    ...
    def put(self, request, id, **kwargs):
        ...
        if email_address.verified:
            ...
            raise BadgrApiException400(
                "Can't make unverified email address the primary email address", 105
            )
