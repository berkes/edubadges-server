...
class TimestampedBadgeInstanceList(BaseEntityListView):
    ...
    def delete(self, request, **kwargs):
        ...
        assertion_timestamp = AssertionTimeStamp.objects.get(
...
class BatchSignAssertions(BaseEntityListView):
    ...
    def post(self, request, **kwargs):
        ...
        assertion_slugs = [
            instance["slug"] for instance in request.data.get("badge_instances")
        ]
        user = request.user
        if not password:
        ...
        for ass_slug in assertion_slugs:  # pun intended
            ...
            if not user in [
                staff.user for staff in assertion_instance.issuer.current_signers
            ]:
                raise ValidationError(
                    "You are not a signer for this issuer: {}".format(
                        assertion_instance.issuer.name
                    )
                )
            if assertion_instance.signature:
                raise ValidationError(
                    "Cannot sign Assertion for Badgeclass {} with recipient identifier {}. Assertion already signed.".format(
                        assertion_instance.badgeclass.name,
                        assertion_instance.recipient_identifier,
                    )
                )
            assertion_instances.append(assertion_instance)

            timestamp = AssertionTimeStamp.objects.get(
                badge_instance=assertion_instance
            )
        ...
        try:
            private_key = tsob.create_new_private_key(
                password, user.current_symmetric_key
            )
        except ValueError as e:
            raise ValidationError(str(e))
        try:
            signed_badges = tsob.sign_badges(
                batch_of_assertion_json,
                private_key,
        ...
        for signed_badge in signed_badges:
            ...
            matching_assertion = [
                ass
                for ass in assertion_instances
                if ass.identifier == signed_badge["plain_badge"]["id"]
            ]
            if len(matching_assertion) > 1:
                raise ValidationError(
                    "Signing failed: Signed json could not be matched to a BadgeInstance"
                )
            matching_assertion = matching_assertion[0]
            matching_assertion.rebake(
                signature=signature, replace_image=True, save=False
            )
        ...
        response = [
            serializer_instance.to_representation(assertion)
...
class BatchAwardEnrollments(VersionedObjectMixin, BaseEntityView):
    ...
    def post(self, request, **kwargs):
        ...
        serializer = serializer_class(
            many=True, data=request.data.get("enrollments"), context=context
        )
