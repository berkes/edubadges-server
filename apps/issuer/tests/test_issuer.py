...
class IssuerAPITest(BadgrTestCase):
    def test_assertion_revoking(self):
        ...
        assertion = self.setup_assertion(
            recipient=student, badgeclass=badgeclass, created_by=teacher1
        )
        post_data = {
    ...
    def test_create_issuer(self):
        ...
        response = self.client.post(
            "/issuer/create", json.dumps(issuer_json), content_type="application/json"
        )
        ...
        created_no_image_issuer = Issuer.objects.get(
            entity_id=response.data["entity_id"]
        )
        self.assertEqual(
            response.data["image_english"],
            created_no_image_issuer.institution.image_url(),
        )
    ...
    def test_may_not_create_issuer(self):
        ...
        self.assertEqual(
            str(response.data["fields"]["error_message"]),
    ...
    def test_create_badgeclass(self):
        ...
        response = self.client.post(
            ...
            json.dumps(badgeclass_json_copy),
            content_type="application/json",
        )
        self.assertEqual(201, response.status_code)
    ...
    def test_create_badgeclass_grondslag_failure(self):
        ...
        self.assertEqual(201, response.status_code)
        teacher1.institution.grondslag_formeel = (
            Institution.GRONDSLAG_GERECHTVAARDIGD_BELANG
        )
    ...
    def test_create_badgeclass_image_too_large(self):
        ...
        self.assertTrue(
            bool(
                str(
                    response.data["fields"]["error_message"]["image"][0][
                        "error_message"
                    ]
                )
            )
    ...
    def test_archive_entity(self):
        ...
        badgeclass_response = self.client.delete(
            "/issuer/badgeclasses/delete/{}".format(badgeclass.entity_id),
            content_type="application/json",
        )
        self.assertEqual(badgeclass_response.status_code, 404)
        issuer_response = self.client.delete(
            "/issuer/delete/{}".format(issuer.entity_id),
            content_type="application/json",
        )
        ...
        query = (
            'query foo{badgeClass(id: "'
            + badgeclass.entity_id
            + '") { entityId name } }'
        )
    ...
    def test_enroll_and_award_badge(self):
        ...
        self.setup_staff_membership(
            teacher1, teacher1.institution, may_award=True, may_read=True
        )
        ...
        self.assertEqual(
            str(failure_response.data[0]["evidence_items"][0]["evidence_url"][0]),
            "Enter a valid URL.",
        )
        award_body["enrollments"][0]["evidence_items"][0]["evidence_url"] = (
            "https://www.valid.com"
        )
    ...
    def test_enrollment_denial(self):
        ...
        response = self.client.put(
            reverse(
                "api_lti_edu_update_enrollment",
                kwargs={"entity_id": enrollment.entity_id},
            )
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_assertion_image_privacy(self):
        ...
        response = self.client.get(
            "/media/uploads/badges/{}".format(os.path.basename(assertion.image.path))
        )
        ...
        )  # teacher belongs to assertion institution
        self.setup_staff_membership(teacher1, teacher1.institution, may_read=True)
        response = self.client.get(
            "/media/uploads/badges/{}".format(os.path.basename(assertion.image.path))
        )
...
class IssuerPublicAPITest(BadgrTestCase):
    def test_get_name_from_recipient_identifer(self):
        ...
        response = self.client.get(
            "/public/assertions/identity/{}/{}".format(eduid_hash, salt)
        )
        self.assertEqual(response.status_code, 404)
        assertion.public = True
        assertion.save()
...
class IssuerModelsTest(BadgrTestCase):
    def test_issuer_uniqueness_constraint(self):
        ...
        self.assertRaises(
            BadgrValidationMultipleFieldError, self.setup_issuer, **setup_issuer_kwargs
        )
        del setup_issuer_kwargs["name_english"]
        self.assertRaises(
            BadgrValidationFieldError, self.setup_issuer, **setup_issuer_kwargs
        )
    ...
    def test_badgeclass_uniqueness_constraints_when_archiving(self):
        ...
        self.assertRaises(
            IntegrityError, self.setup_badgeclass, **setup_badgeclass_kwargs
        )
    ...
    def test_badgeinstance_get_json(self):
        ...
        badgeclass = self.setup_badgeclass(issuer=issuer)
        evidence_items = [
            {"evidence_url": "http://valid.com", "narrative": "Some narrative"}
        ]
        assertion = self.setup_assertion(
            student,
            badgeclass,
...
class IssuerSchemaTest(BadgrTestCase):
    ...
    def test_badgeclass_schema(self):
        ...
        self.assertTrue(
            bool(
                response["data"]["badgeClasses"][0]["terms"]["termsUrl"][0]["language"]
            )
        )
        self.assertEqual(
            len(response["data"]["badgeClasses"][0]["terms"]["termsUrl"]), 4
        )
