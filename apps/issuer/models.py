...
from mainsite.exceptions import (
    ...
    BadgrValidationMultipleFieldError,
)
...
class Issuer(
    ...
    name_dutch = models.CharField(
        max_length=512, null=True
    )  # or this one, must be supplied to pass save() method
    ...
    faculty = models.ForeignKey(
        "institution.Faculty", on_delete=models.CASCADE, blank=False, null=False
    )

    @property
    def description(self):
        return self.return_value_according_to_language(
            self.description_english, self.description_dutch
        )

    @property
    def name(self):
        return self.return_value_according_to_language(
            self.name_english, self.name_dutch
        )

    @property
    def image(self):
        image = self.return_value_according_to_language(
            self.image_english, self.image_dutch
        )
        if not image:
    ...
    def validate_unique(self, exclude=None):
        if not self.archived:
            if self.name_dutch and self.name_english:
                query = Q(name_english=self.name_english) | Q(
                ...
                raise BadgrValidationMultipleFieldError(
                    [
                        [
                            "name_english",
                            "Either Dutch or English name is required",
                            913,
                        ],
            ...
            issuer_same_name = (
                self.__class__.objects.filter(
                    query, faculty=self.faculty, archived=False
                )
                .exclude(pk=self.pk)
                .first()
            )
            if issuer_same_name:
                name_english_the_same = (
                    issuer_same_name.name_english == self.name_english
                    and bool(issuer_same_name.name_english)
                )
                name_dutch_the_same = (
                    issuer_same_name.name_dutch == self.name_dutch
                    and bool(issuer_same_name.name_dutch)
                )
    ...
    def save(self, *args, **kwargs):
        if not self.name_english and not self.name_dutch:
            raise BadgrValidationError(
                "Either English or Dutch name must be supplied", 999
            )
    ...
    def get_badgeclasses(self, user, permissions):
        return [
            ...
            if bc.has_permissions(user, permissions)
        ]
    ...
    def get_json(
        ...
        if self.image:
            image_url = OriginSetting.HTTP + reverse(
    ...
    def cached_badgrapp(self):
        id = (
            self.badgrapp_id
            if self.badgrapp_id
            else getattr(settings, "BADGR_APP_ID", 1)
        )
...
class BadgeClass(
    ...
    quality_assurance_name = models.CharField(
        max_length=254, blank=True, null=True, default=None
    )
    quality_assurance_url = models.CharField(
        max_length=254, blank=True, null=True, default=None
    )
    quality_assurance_description = models.TextField(
    ...
    badge_class_type = models.CharField(
        max_length=254,
        choices=BADGE_CLASS_TYPE_CHOICES,
    ...
    staff = models.ManyToManyField(
        "badgeuser.BadgeUser", through="staff.BadgeClassStaff"
    )
    ...
    def validate_unique(self, exclude=None):
        if not self.archived:
            if (
                self.__class__.objects.filter(
                    name=self.name, issuer=self.issuer, archived=False
                )
                .exclude(pk=self.pk)
                .exists()
            ):
                raise IntegrityError(
                    "Badgeclass with this name already exists in the same issuer."
                )
    ...
    def cached_pending_direct_awards(self):
        return DirectAward.objects.filter(
            badgeclass=self, status=DirectAward.STATUS_UNACCEPTED
        )
    ...
    def _get_terms(self):
        ...
        if not terms:
            raise ValueError(
                f"Institution {self.institution.identifier} has no terms. This is required."
            )
        if self.formal:
            return [
                term
                for term in terms
                if term.terms_type == term.__class__.TYPE_FORMAL_BADGE
            ][0]
        return [
            term
            for term in terms
            if term.terms_type == term.__class__.TYPE_INFORMAL_BADGE
        ][0]
    ...
    def cached_pending_enrollments(self):
        ...
        return StudentsEnrolled.objects.filter(
            badge_class=self, badge_instance=None, denied=False
        )

    @property
    def assertions_count(self):
        return BadgeInstance.objects.filter(
            badgeclass=self, revoked=False, acceptance="Accepted"
        ).count()
    ...
    def issue(
        ...
        **kwargs,
        ...
        if (
            not recipient.validated_name
            and enforce_validated_name
            and not self.award_non_validated_name_allowed
        ):
            raise serializers.ValidationError(
                "You need a validated_name from an Institution to issue badges."
            )
        ...
        if send_email:
            recipient.email_user(
                subject="Je hebt een edubadge ontvangen! You received an edubadge!",
    ...
    def issue_signed(
        ...
        **kwargs,
        ...
        if not perms["may_sign"]:
            raise serializers.ValidationError(
                "You do not have permission to sign badges for this badgeclass."
            )
        assertion = BadgeInstance.objects.create(
            badgeclass=self,
            recipient_identifier=recipient.get_recipient_identifier(),
...
class BadgeInstance(
    BaseAuditedModel, ImageUrlGetterMixin, BaseVersionedEntity, BaseOpenBadgeObjectModel
):
    entity_class_name = "Assertion"

    issued_on = models.DateTimeField(blank=False, null=False, default=timezone.now)

    public_key_issuer = models.ForeignKey(
        "signing.PublicKeyIssuer", on_delete=models.PROTECT, null=True, default=None
    )

    identifier = models.CharField(
        max_length=255, null=True, default=None
    )  # the uuid used to ID signed assertions
    ...
    issuer = models.ForeignKey(
        Issuer, on_delete=models.PROTECT, blank=False, null=False
    )
    ...
    recipient_identifier = models.CharField(
        max_length=512, blank=False, null=False, db_index=True
    )

    image = models.FileField(
        upload_to="uploads/badges", blank=True, null=True, db_index=True
    )
    ...
    salt = models.CharField(
        max_length=254, blank=True, null=True, default=None, db_index=True
    )
    ...
    def get_json(
        ...
        if signed:
            if expand_issuer != True or expand_badgeclass != True:
                raise ValueError(
                    "Must expand issuer and badgeclass if signed is set to true."
                )
            if not public_key_issuer:
                raise ValueError(
                    "Must add a public key issuer object (address) if signed is set to true."
                )
            if public_key_issuer.issuer != self.issuer:
                raise ValueError(
                    "Public key issuer objects does not match assertion issuer"
                )
        ...
        if expand_badgeclass:
            json["badge"] = badge_class.get_json(
                obi_version=obi_version, include_extra=include_extra
            )
            if signed:
                json["badge"]["id"] = badge_class.get_url_with_public_key(
                    public_key_issuer
                )
            if expand_issuer:
            ...
            if expand_user:
                ...
                json["badge"]["endorsements"] = [
                    BadgeClassJson.endorsement_to_json(bc)
                    for bc in badge_class.cached_endorsements()
                ]
    ...
    def get_filtered_json(
        ...
        return super(BadgeInstance, self).get_filtered_json(
            excluded_fields=excluded_fields
        )
...
class BadgeInstanceCollection(BaseAuditedModel, BaseVersionedEntity, CacheModel):
    ...
    def validate_unique(self, exclude=None):
        if (
            self.__class__.objects.filter(name=self.name, user=self.user)
            .exclude(pk=self.pk)
            .exists()
        ):
            raise BadgrValidationFieldError(
