...
class BadgeCheckHelper(object):
    ...
    def get_or_create_assertion(
        ...
        recipient_identifier = report.get("recipientProfile", {}).get(
            "id", None
        )  # first use 'id' as the recipient_identifier type
        if (
            not recipient_identifier
        ):  # the assertion might instead have the 'email' recipient_identifier type
        ...
        with transaction.atomic():
            issuer, issuer_created = Issuer.objects.get_or_create_from_ob2(
                issuer_obo, original_json=original_json.get(issuer_obo.get("id"))
            )
