...
class IssuerSerializer(
    ...
    description_english = StripTagsCharField(
        max_length=16384, required=False, allow_null=True, allow_blank=True
    )
    description_dutch = StripTagsCharField(
        max_length=16384, required=False, allow_null=True, allow_blank=True
    )
    url_english = serializers.URLField(
        max_length=1024, required=False, allow_null=True, allow_blank=True
    )
    url_dutch = serializers.URLField(
        max_length=1024, required=False, allow_null=True, allow_blank=True
    )
        ...
        if not data.get("name_dutch", False) and not data.get("name_english", False):
            e = OrderedDict(
                [
                    (
                        "name_dutch",
                        [ErrorDetail("Dutch or English name is required", code=912)],
                    )
                ]
            )
            ...
            e = OrderedDict(
                [
                    (
                        "name_english",
                        [ErrorDetail("English or Dutch name is required", code=924)],
                    )
                ]
            )
            errors = OrderedDict(chain(errors.items(), e.items()))
        if not data.get("description_dutch", False) and not data.get(
            "description_english", False
        ):
            e = OrderedDict(
                [
                    (
                        ...
                        [
                            ErrorDetail(
                                "Dutch or English description is required", code=913
                            )
                        ],
                    )
                ]
            ...
            e = OrderedDict(
                [
                    (
                        ...
                        [
                            ErrorDetail(
                                "English or Dutch description is required", code=925
                            )
                        ],
                    )
                ]
        ...
        if not data.get("url_dutch", False) and not data.get("url_english", False):
            e = OrderedDict(
                [
                    (
                        "url_dutch",
                        [ErrorDetail("Dutch or English url is required", code=915)],
                    )
                ]
            )
            errors = OrderedDict(chain(errors.items(), e.items()))
            e = OrderedDict(
                [
                    (
                        "url_english",
                        [ErrorDetail("English or Dutch url is required", code=923)],
                    )
                ]
            )
            errors = OrderedDict(chain(errors.items(), e.items()))
        ...
        if not self.root.initial_data.get("isMicroCredentials", True):
            if not target_url or len(target_url.strip()) == 0:
                raise ValidationError(
                    detail="This field may not be blank.", code="blank"
                )
        return target_url
...
class BadgeClassSerializer(
    ...
    award_non_validated_name_allowed = serializers.BooleanField(
    ...
    description = StripTagsCharField(
        max_length=16384, required=False, convert_null=True, allow_blank=True
    )
    badge_class_type = StripTagsCharField(
        required=True, allow_blank=False, allow_null=False
    )
    participation = StripTagsCharField(
        required=False, allow_blank=True, allow_null=True
    )
    assessment_type = StripTagsCharField(
        required=False, allow_blank=True, allow_null=True
    )
    ...
    alignments = AlignmentItemSerializer(
    ...
    tags = PrimaryKeyRelatedField(
        many=True, queryset=BadgeClassTag.objects.all(), required=False
    )
    ...
    def validate(self, data):
        ...
        for extension in extensions:
            if not extension_items.get(f"extensions:{extension}"):
                errors[f"extensions.{extension}"] = ErrorDetail(
                    "This field may not be blank.", code="blank"
                )
        if errors:
    ...
    def update(self, instance, validated_data):
        has_unrevoked_assertions = (
            instance.assertions
            and len([ass for ass in instance.assertions if not ass.revoked]) > 0
        )
        ...
        for key, value in validated_data.items():
            if key not in many_to_many_keys and (
                not has_unrevoked_assertions or key in allowed_keys
            ):
    ...
    def create(self, validated_data, **kwargs):
        ...
        if user_permissions["may_create"]:
            if (
                validated_data["formal"]
                and not validated_data["issuer"].faculty.institution.grondslag_formeel
            ):
                raise BadgrValidationError(
            ...
            if (
                not validated_data["formal"]
                and not validated_data["issuer"].faculty.institution.grondslag_informeel
            ):
                raise BadgrValidationError(
...
class BadgeInstanceSerializer(OriginalJsonSerializerMixin, serializers.Serializer):
    ...
    def validate(self, data):
        ...
        if badgeclass.narrative_required and not data.get("narrative"):
            raise BadgrValidationError(
                error_code=999, error_message="Narrative is required"
            )
        if badgeclass.evidence_required and not data.get("evidence_items"):
            raise BadgrValidationError(
                error_code=999, error_message="Evidence is required"
            )
        ...
        if hashed is None:
            ...
            if recipient_type in (
                BadgeInstance.RECIPIENT_TYPE_URL,
    ...
    def to_representation(self, instance):
        representation = super(BadgeInstanceSerializer, self).to_representation(
            instance
        )
        representation["json"] = instance.get_json(
            obi_version="1_1", use_canonical_id=True
        ...
        if self.context.get("include_issuer", False):
            representation["issuer"] = IssuerSerializer(
        ...
        if self.context.get("include_badge_class", False):
            representation["badge_class"] = BadgeClassSerializer(
    ...
    def create(self, validated_data):
        ...
        badgeclass.cached_pending_direct_awards().filter(
            eppn__in=enrollment.user.eppns
        ).delete()
        return assertion
...
class BadgeInstanceCollectionSerializer(serializers.Serializer):
    ...
    badge_instances = PrimaryKeyRelatedField(
        many=True, queryset=BadgeInstance.objects.all(), required=False
    )
