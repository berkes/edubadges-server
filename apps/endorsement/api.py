...
def send_notifications(endorsement: Endorsement, current_user):
    user_notifications = BadgeClassUserNotification.objects.filter(
        badgeclass=endorsement.endorser
    ).all()
    ...
    def post(self, request, **kwargs):
        ...
        if serializer.is_valid():
            ...
            thread = threading.Thread(
                target=send_notifications, args=(endorsement, request.user)
            )
            thread.start()
...
class EndorsementDetail(BaseEntityDetailView):
    ...
    def put(self, request, **kwargs):
        ...
        if (
            not endorser_permissions["may_award"]
            and not endorsee_permissions["may_award"]
        ):
        ...
        if new_status == Endorsement.STATUS_REVOKED:
            ...
            EmailMessageMaker.reject_approve_endorsement_mail(
                request.user, endorsement, True
            )
        elif new_status == Endorsement.STATUS_REJECTED:
            endorsement.rejection_reason = request.data["rejection_reason"]
            EmailMessageMaker.reject_approve_endorsement_mail(
                request.user, endorsement, False
            )
        endorsement.save()
