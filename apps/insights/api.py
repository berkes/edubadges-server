    ...
    def post(self, request, **kwargs):
        ...
        if not total:
            ...
            if start_of_year.isoweekday() > 1:
                start_of_year = start_of_year + timedelta(
                    days=(7 + 1) - start_of_year.isoweekday()
                )
            end_of_year = current_date.replace(year=year, month=12, day=31)
            if end_of_year.isoweekday() > 1:
                end_of_year = end_of_year + timedelta(
                    days=(7 + 1) - end_of_year.isoweekday()
                )
        ...
        if not total:
            assertions_query_set = assertions_query_set.filter(
                created_at__gte=start_of_year
            ).filter(created_at__lt=end_of_year)
        if filter_by_institution:
            assertions_query_set = assertions_query_set.filter(
                issuer__faculty__institution=institution
            )
        if not filter_by_institution and not include_surf:
            assertions_query_set = assertions_query_set.exclude(
        ...
        enrollments_query_set = (
            StudentsEnrolled.objects.filter(
                Q(badge_instance_id__isnull=True) | Q(denied=True)
            )
            .values(
                "denied",
                "badge_class_id",
        ...
        if not total:
            enrollments_query_set = enrollments_query_set.filter(
                date_created__gte=start_of_year
            ).filter(date_created__lt=end_of_year)
        if filter_by_institution:
            enrollments_query_set = enrollments_query_set.filter(
                badge_class__issuer__faculty__institution=institution
            )
        if not filter_by_institution and not include_surf:
        ...
        if filter_by_institution:
            badge_class_query = badge_class_query.filter(
                issuer__faculty__institution=institution
            )
        if not filter_by_institution and not include_surf:
            badge_class_query = badge_class_query.exclude(
                issuer__faculty__institution=surf_institution
            )
        ...
        if not filter_by_institution and not include_surf:
            backpack_query = backpack_query.exclude(
                schac_home__iexact=surf_institution.identifier
            )
...
class InstitutionBadgesOverview(APIView):
    ...
    def get(self, request, **kwargs):
        ...
        institution_part = (
            "" if is_super_user else f"and ins.id = {request.user.institution.id}"
        )
...
where  status <>  'Deleted' and status <> 'Revoked' and status <> 'Scheduled' group by badgeclass_id;
            ...
            for key, val in grouped_assertions:
                ...
                direct_awards_accepted = sum(
                    [
                        b["backpack_count"]
                        for b in values
                        if b["award_type"] == "direct_award" and not b["revoked"]
                    ]
                )
                direct_awards_assertions_revoked = sum(
                    [
                        b["backpack_count"]
                        for b in values
                        if b["award_type"] == "direct_award" and b["revoked"]
                    ]
                )
                direct_awards_rejected_or_unaccepted = sum(
                    [
                        ...
                        if str(da["badgeclass_id"]) == str(key)
                    ]
                )
                total_da_count = (
                ...
                results.append(
                    {
                        ...
                        "Total edubadges in backpack": sum(
                            [b["backpack_count"] for b in values if not b["revoked"]]
                        ),
                        ...
                        "Requested accepted": sum(
                            [
                                b["backpack_count"]
                                for b in values
                                if b["award_type"] == "requested" and not b["revoked"]
                            ]
                        ),
                        ...
                        "Requested revoked": sum(
                            [
                                b["backpack_count"]
                                for b in values
                                if b["award_type"] == "requested" and b["revoked"]
                            ]
                        ),
                        "Public": sum(
                            [b["backpack_count"] for b in values if b["public_badge"]]
                        ),
                        "Claim-rate": claim_rate(
                            (total_da_count - direct_awards_assertions_revoked),
                            direct_awards_accepted,
                        ),
            ...
            sorted_results = sorted(
                results, key=lambda a: (a["Institution name"], a["BadgecClass name"])
            )
...
class IssuerMembers(APIView):
    ...
    def get(self, request, **kwargs):
        ...
        institution_part = (
            "" if is_super_user else f"ins.id = {request.user.institution.id} and "
        )
...
where {institution_part} si.may_update is not null and i.id is not null order by i.id;
            ...
            for row in issuer_overview:
                results.append(
                    {
                        "issuer_name": row["issuer_name_en"]
                        if row["issuer_name_en"]
                        else row["issuer_name_nl"],
                        "email": row["email"],
