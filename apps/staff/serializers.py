...
class BaseStaffCreateSerializer(BaseStaffSerializer):
    ...
    def _base_create(self, validated_data, object_name, object_class):
        ...
        if created_by.is_user_within_scope(validated_data["user"]):
            perms_allowed_to_assign = validated_data[object_name].get_permissions(
                created_by
            )
            ...
            new_staff_membership.user.email_user(
                subject="You have been added to {} {}".format(determiner, entity_name),
                html_message=message,
            )
            logger.event(
                badgrlog.PermissionCreatedEvent(
                    staff_instance=new_staff_membership, request=self.context["request"]
                )
            )
...
class InstitutionStaffSerializer(BaseStaffCreateSerializer):
    ...
    def create(self, validated_data):
        institution_staff_membership = self._base_create(
            validated_data, "institution", InstitutionStaff
        )
...
class FacultyStaffSerializer(BaseStaffCreateSerializer):
    ...
    def create(self, validated_data):
        faculty_staff_membership = self._base_create(
            validated_data, "faculty", FacultyStaff
        )
        ...
        [
            staff.delete()
            for staff in user.badgeclassstaff_set.all()
            if staff.badgeclass.issuer.faculty == fac
        ]
        [
            staff.delete()
            for staff in user.issuerstaff_set.all()
            if staff.issuer.faculty == fac
        ]
...
class IssuerStaffSerializer(BaseStaffCreateSerializer):
    ...
    def create(self, validated_data):
        issuer_staff_membership = self._base_create(
            validated_data, "issuer", IssuerStaff
        )
        ...
        [
            staff.delete()
            for staff in user.badgeclassstaff_set.all()
            if staff.badgeclass.issuer == iss
        ]
