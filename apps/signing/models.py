...
class AssertionTimeStamp(models.Model):
    ...
    def submit_assertion(self):
        ...
        canonicalized_json, hashed_json, hash_id_nodes = (
            timestamping.submit_json_for_timestamping(assertion_json)
        )
    ...
    def resubmit_json(self):
        canonicalized_json, hashed_json, hash_id_nodes = (
            timestamping.submit_json_for_timestamping(self.original_json)
        )
        self.hash_id_nodes = json.dumps(hash_id_nodes)
        self.hash = hashed_json
        self.save()
...
class PublicKeyIssuer(BaseVersionedEntity, models.Model):
    ...
    public_key = models.ForeignKey(
        "signing.PublicKey", on_delete=models.PROTECT, null=True, default=None
    )
