...
urlpatterns = [
    ...
    url(
        r"^context/(?P<launch_id>[\w-]+)/$",
        csrf_exempt(get_lti_context),
        name="lti_context",
    ),
    url(r"^grades/(?P<launch_id>[\w-]+)/$", csrf_exempt(get_grades), name="lti_grades"),
    url(
        r"^members/(?P<launch_id>[\w-]+)/$",
        csrf_exempt(get_members),
        name="lti_members",
    ),
    url(r"^course/create$", LtiCourseDetailList.as_view(), name="lti_course_create"),
    url(
        r"^course/delete/(?P<entity_id>[^/]+)$",
        LtiCourseDetail.as_view(),
        name="lti_course_delete",
    ),
]
