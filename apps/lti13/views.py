...
def launch(request):
    ...
    institution = Institution.objects.get(
        identifier=registration.get_institution_identifier()
    )
    ...
    try:
        user = BadgeUser.objects.get(
            email=email, is_teacher=True, institution=institution
        )
    ...
    social_login = SocialLogin(
        account=social_account, email_addresses=[email for email in user.email_items]
    )
