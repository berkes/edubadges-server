...
class Query(object):
    ...
    lti_tool = graphene.Field(
        LtiToolType, client_id=graphene.String(), issuer=graphene.String()
    )

    def resolve_lti_course(self, info, **kwargs):
        badge_class_id = kwargs.get("badge_class_id")
        if badge_class_id:
            return LtiCourse.objects.filter(
                badgeclass__entity_id=badge_class_id
            ).first()
        return None
