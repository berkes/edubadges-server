...
class LtiTool(models.Model):
    ...
    use_by_default = models.BooleanField(
        default=False,
        help_text=_(
            "This iss config will be used in case " "if client-id was not passed"
        ),
    )
    ...
    auth_audience = models.CharField(
        ...
        blank=True,
        help_text=_(
            "The platform's OAuth2 Audience (aud). " "Usually could be skipped"
        ),
    )
    key_set_url = models.CharField(
        ...
        blank=True,
        help_text=_(
            "The platform's JWKS endpoint. " "Value provided by LTI 1.3 Platform"
        ),
        validators=[URLValidator()],
    )
