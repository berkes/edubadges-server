...
class MainGrapheneTest(BadgrTestCase):
    def test_pagination(self):
        ...
        end_cursor = response["data"]["badgeClass"]["assertionsPaginated"]["pageInfo"][
            "endCursor"
        ]
        assertions_entity_ids_1 = [
            edge["node"]["entityId"]
            for edge in response["data"]["badgeClass"]["assertionsPaginated"]["edges"]
        ]
        ...
        self.assertFalse(
            all(
                entity_id in assertions_entity_ids_1
                for entity_id in assertions_entity_ids_2
            )
        )
