...
class SetupHelper(object):
    ...
    def add_eduid_socialaccount(self, user):
        random_eduid = "urn:mace:eduid.nl:1.0:d57b4355-c7c6-4924-a944-6172e31e9bbc:{}c14-b952-4d7e-85fd-{}ac5c6f18".format(
        ...
        socialaccount = SocialAccount(
            extra_data=extra_data, uid=random_eduid, provider="edu_id", user=user
        )
        socialaccount.save()
    ...
    def authenticate(self, user):
        ...
        return self.client._login(
            user, backend="oauth2_provider.backends.OAuth2Backend"
        )
    ...
    def setup_teacher(
        ...
        user = self.setup_user(
            first_name, last_name, authenticate, institution=institution, email=email
        )
    ...
    def setup_student(
        ...
        first_name = (
            string_randomiser("student_first_name") if not first_name else first_name
        )
        last_name = (
            string_randomiser("student_last_name") if not last_name else last_name
        )
        user = self.setup_user(first_name, last_name, authenticate, institution=None)
        self.add_eduid_socialaccount(user)
        affiliations = [
            {"schac_home": inst.identifier, "eppn": string_randomiser("eppn")}
    ...
    def setup_direct_award_bundle(self, badgeclass, direct_awards=[], **kwargs):
        dab = DirectAwardBundle.objects.create(
            badgeclass=badgeclass, initial_total=len(direct_awards), **kwargs
        )
    ...
    def setup_direct_award(self, badgeclass, **kwargs):
        ...
        if not kwargs.get("bundle", False):
            kwargs["bundle"] = DirectAwardBundle.objects.create(
                badgeclass=badgeclass, initial_total=1
            )
        else:
    ...
    def _setup_institution_terms(self, institution):
        formal_badge, _ = Terms.objects.get_or_create(
            institution=institution, terms_type=Terms.TYPE_FORMAL_BADGE
        )
        ...
        informal_badge, _ = Terms.objects.get_or_create(
            institution=institution, terms_type=Terms.TYPE_INFORMAL_BADGE
        )
        TermsUrl.objects.get_or_create(
            terms=informal_badge,
            language=TermsUrl.LANGUAGE_ENGLISH,
            excerpt=False,
            url="https://raw.githubusercontent.com/edubadges/privacy/master/university-example.org/formal-edubadges-principles-en.md",
        )
    ...
    def setup_general_terms(self):
        terms_of_service, _ = Terms.objects.get_or_create(
            version=1, institution=None, terms_type=Terms.TYPE_TERMS_OF_SERVICE
        )
        TermsUrl.objects.get_or_create(
        ...
        terms_service_agreement_student, _ = Terms.objects.get_or_create(
            version=1, institution=None, terms_type=Terms.TYPE_SERVICE_AGREEMENT_STUDENT
        )
        TermsUrl.objects.get_or_create(
            terms=terms_service_agreement_student,
            url="https://raw.githubusercontent.com/edubadges/privacy/master/university-example.org/formal-edubadges-principles-en.md",
            language=TermsUrl.LANGUAGE_DUTCH,
        )
        TermsUrl.objects.get_or_create(
        ...
        terms_service_agreement_employee, _ = Terms.objects.get_or_create(
            version=1,
            institution=None,
            terms_type=Terms.TYPE_SERVICE_AGREEMENT_EMPLOYEE,
        )
        TermsUrl.objects.get_or_create(
            terms=terms_service_agreement_employee,
            url="https://raw.githubusercontent.com/edubadges/privacy/master/university-example.org/formal-edubadges-principles-en.md",
            language=TermsUrl.LANGUAGE_DUTCH,
        )
        TermsUrl.objects.get_or_create(
    ...
    def setup_faculty(self, **kwargs):
        ...
        if not kwargs.get("name_english", False) and not kwargs.get(
            "name_dutch", False
        ):
    ...
    def setUp(self):
        ...
        try:
            ...
            self.badgr_app = BadgrApp.objects.create(
                name="test cors", cors="localhost:8000"
            )
