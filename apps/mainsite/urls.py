...
urlpatterns = [
    ...
    url(
        r"^signing/", include(("signing.api_urls", "signing"), namespace="signing_apis")
    ),
...
if getattr(settings, "DEBUG_STATIC", True):
    ...
    static_url = static_url.replace(
        getattr(settings, "HTTP_ORIGIN_MEDIA", "http://localhost:8000"), ""
    )
...
urlpatterns += [
    ...
    path(
        "api/schema/swagger-ui/oauth2-redirect.html",
