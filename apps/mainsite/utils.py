...
class EmailMessageMaker:
    ...
    def create_endorsement_requested_mail(current_user, user, endorsement):
        ...
        email_vars = {
            ...
            "ui_url": urllib.parse.urljoin(
                settings.UI_URL, f"badgeclass/{endorsement.endorser.entity_id}/endorsed"
            ),
            "ui_url_notifications": urllib.parse.urljoin(
                settings.UI_URL, "notifications"
            ),
            "user": user,
    ...
    def create_enrolment_notification_mail(badge_class, student, created_enrollment):
        ...
        email_vars = {
            ...
            "ui_url": urllib.parse.urljoin(
                settings.UI_URL, f"badgeclass/{badge_class.entity_id}/enrollments"
            ),
            "ui_url_notifications": urllib.parse.urljoin(
                settings.UI_URL, "notifications"
            ),
...
def add_watermark(uploaded_image, is_svg):
    ...
    preferred_width = int(
        math.sqrt(int(math.pow(img.size[0], 2)) + int(math.pow(img.size[1], 2)))
    )
...
def scrub_svg_image(uploaded_image):
    ...
    for tag_name in MALICIOUS_SVG_TAGS:
        elements_to_strip.extend(
            root.findall("{{{ns}}}{tag}".format(ns=SVG_NAMESPACE, tag=tag_name))
        )
