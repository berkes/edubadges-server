...
class OIDCAuthentication(BaseAuthentication):
    def authenticate(self, request):
        ...
        if response.status_code != 200:
            logger.info(
                f"OIDCAuthentication bad response {response.status_code} {response.json()}"
            )
            return None
        ...
        if "email" in introspect_json:
            user = BadgeUser.objects.get(
                email=introspect_json["email"], is_teacher=True
            )
            ...
            logger.info(
                f"OIDCAuthentication institution {institution} client_id {client_id}"
            )
