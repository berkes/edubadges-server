...
def accept_terms(user):
    if user.is_teacher:
        ...
        for term in terms:
            terms_agreement, _ = TermsAgreement.objects.get_or_create(
                user=user, terms=term
            )
...
def create_admin(
    username, email, first_name, last_name, institution_name, uid, perms=all_perms
):
...
def create_teacher(
    username, email, first_name, last_name, institution_name, uid, perms=no_perms
):
    user, _ = BadgeUser.objects.get_or_create(
        username=username,
        email=email,
...
def create_student(username, first_name, last_name, email, uid):
    ...
    social_account, _ = SocialAccount.objects.get_or_create(
        provider="edu_id", uid=uid, user=user
    )
