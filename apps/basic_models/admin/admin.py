...
class AutoGroupMeta(ModelAdmin):
    ...
    def get_readonly_fields(self, request, obj=None):
        ...
        if obj:
            ...
            fields_which_exist = [
                field for field in readonly_fields if hasattr(obj, field)
            ]
    ...
    def get_fieldsets(self, request, obj=None):
        ...
        fields = [
            ...
            if hasattr(obj, field) and field not in exclude
        ]

        if fields:
            meta_grouped.append(("Meta", {"fields": fields, "classes": ("collapse",)}))
...
class ActiveModelAdmin(ModelAdmin):
    ...
    def _set_objects_active(self, request, queryset, active):
        ...
        self.message_user(
            request,
            _("Successfully %(prefix)sactivated %(count)d %(items)s.")
            % {
                "prefix": "" if active else "de",
                "count": count,
...
class OneActiveAdmin(ModelAdmin):
    ...
    def duplicate(self, request, queryset):
        for object in queryset:
            object.clone()
        duplicate.short_description = ugettext_lazy(
